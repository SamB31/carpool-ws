...
const { getNextStation, adjustStations } = require("./stationService");
...
const checkIn = async (familyId) => {
  try {
    ...
    // Check if any family member is in after school care
    const familyInAfterSchoolCare = familyMembers.some(member => member.inAfterSchoolCare);

    if (familyInAfterSchoolCare) {
      return { success: false, status: 400, message: `${lastName} is in after school care` };
    }
...
const checkOut = async (childIds) => {
  try {
    ...
    const lastNames = [
      ...new Set(checkedInMembers.map((member) => member.lastName)),
    ];
...
const formatMessage = async () => {
  try {
    ...
    if (formattedData.length > 1) {
      formattedData.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    }
